...
words_to_remove = ['burger', 'sandwich']
for item in menu_dict.keys():
    ...
    # Also add versions of the item without words like 'burger'
    item_doc_filtered = [token for token in item_doc if token.lemma_ not in words_to_remove]
    lemmatized_item_filtered = ' '.join([token.lemma_ for token in item_doc_filtered])
    if lemmatized_item_filtered != lemmatized_item:
        lemmatized_menu_items[lemmatized_item_filtered] = item
